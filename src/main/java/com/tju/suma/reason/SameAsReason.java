...
public class SameAsReason {
    public static List<HashSet<Integer>> equiPool = new CopyOnWriteArrayList<>();
...
//        synchronized (object_loopRsRpFindRo) {
            ...
            for (int tmp22 : tmpPool) {
                if (tmp22 != tmp) {
                    ...
                    while (rpRoList.hasNext()) {
                        ...
                        DicRdfDataMap.addNewRdfDataBeanParallel(tmp22, rp, ro);
                    }
                }
            ...
            if (tmp22 != tmp) {
                ...
                while (rsRpList.hasNext()) {
                    ...
                    if (!ThreeKeyMap.checkDuplicate(rs, rp, tmp22)) {
                        DicRdfDataMap.addNewRdfDataBeanParallel(rs, rp, tmp22);
                    }
                }
            }
...
//        synchronized (object_loopRpRoFindRs){
...
//        synchronized (object_comEquiPool) {
...
//        synchronized (object_reFreshEquiRepre) {
...
//        }
    ...
    public static int findEquivPoolIndex(int rs) {
        return equiPoolIndex.getOrDefault(rs, 0);
